...
/*
 * This is the parser class used to take any code written in our language and transforms it into 
 * a readable Java file that executes. It will execute based upon a dynamic type check basis.
 */
public class Parser {
	// Pattern items for the Parser
	...
	/*
	 * Parses the input command and checks for type of expression Returns an array
	 * of tokens which contains the entire split expression Inserts a header string
	 * to indicate what was entered and successfully parsed This is used by the
	 * translator to figure out what to do
	 */
	...
	/*
	 * This function takes the given function and creates a new save point for the
	 * given function. It acts as a function creation for the given String.
	 */
	...
	/*
	 * Creates a loop tracker for the current while loop
	 */
	...
	/*
	 * Checks to see if it is a print function call
	 */
	...
	/*
	 * Checks for a function call to make a new function tracker
	 */
	...
	/*
	 * Checks for a function creation with any extra arguments
	 */
	...
	/*
	 * Checks for an Iffy Command for a if statement
	 */
	...
	public static boolean isOtherIffyCommand(String command) {
		...
		return (isOtherWord(el) && isIffyWord(iffy) && isBooleanExpression(bool_e));
	}

	/*
	 * Checks for an 'other' line/command
	 */
	public static boolean isOtherCommand(String command) {
		String[] tokens = command.split("\\s+");
		if (tokens.length != 1) {
			return false;
		}
		return (isOtherWord(tokens[0]));
	}

	// Is command word Check
	...
	// Checks for the word "then"
	private static boolean isOtherWord(String command) {
	...
	private static boolean isBooleanVariableOperatorAssignment(String command) {
		/*
		 * Function checks the var name and '=', then checks the rest using
		 * isBooleanExpression()..... - NOTE: the check for just a single bool or single
		 * var (from other func) is NOT relevant here; isVariableAssignment() will cover
		 * it first. - slices list, rejoins the section of the string command, and
		 * checks if that section (after the '=') evaluates to a boolean by running func
		 */
		String[] tokens = command.split("\\s+");
		if (tokens.length < 3) {
			return false;
		...
		String after_eq_JOINED = String.join(" ", expr_after_eq_op); // string of everything after the var name and the
																		// '='
		// true if var name is legal, has '=' after, and everything after is a valid
		// boolean expression
	...
	private static boolean isComparisonExpression(String command) {
		...
		return ((isVariable(var1) || isStrStatement(var1)) && isComparisonOperator(operator)
				&& (isVarOrIntOrBool(var2) || isStrStatement(var2)));
	}

	// the given expression results in a boolean.
	private static boolean isBooleanExpression(String command_trunc) {
		/*
		 * Returns true if the given section of the command is something that results in
		 * a boolean. Intended to be used for: - boolean variable assignment - iffy
		 * statements (iffy <bool_exr> then...)
		 * 
		 * NOTE: this does NOT mean it's only booleans; ALSO RETURNS TRUE WHEN GIVEN A
	...
	/*
	 * Checks to see which kind of variable is being used in the code.
	 */
