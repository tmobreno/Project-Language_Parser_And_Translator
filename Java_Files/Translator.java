...
/*
 * The translator class will be responsible for keeping the parser item and will make sure 
 * to execute all of the needed commands and parser functions when translating the given code.
 */
public class Translator {
	...
	// Contains every passes function
	...
	// This will contain the actual program to translate our language into a
	...
	private static void performIffy(String allLines[]) {
		...
		for (String line : bodyToEnd) {
			if (Parser.isOtherIffyCommand(line.trim())) {
				if (hasHadTrue) {
					ignore = true;
				} else {
			...
			} else if (Parser.isOtherCommand(line)) {
				ignore = hasHadTrue;
			} else {
				if (!ignore) {
					String[] parsing = Parser.parseCommand(line);
	...
	// This executes the functions given
	...
	// This takes the loop and passes it to check what command is being executed
	...
	// Changes the assigned variable for what is needed
	...
	// Performs boolean operation evaluation (or(/) + and(&)).
	...
	private static Boolean newComparisonCommand(String cmd[]) {
		...
		if ((isInteger(cmd[0]) || getObjectAsInteger(cmd[0]) != null)
				&& (isInteger(cmd[2]) || getObjectAsInteger(cmd[2]) != null)) {
			return newComparisonCommandInts(cmd);
		} else if ((isBool(cmd[0]) || getObjectAsBool(cmd[0]) != null)
				&& (isBool(cmd[2]) || getObjectAsBool(cmd[2]) != null)) {
	...
	// Checks if the str is an integer
	...
	// Checks if the str is a Boolean
